...
\centering
\begin{tikzpicture}[
    % text width, not just minimum width, is what makes every box's width identical and
    % guaranteed regardless of its own label's length: minimum width alone is only a lower
    % bound, so a node with a long enough label silently grows past it - text width instead
    % caps the label's own layout box, wrapping onto more lines rather than widening, so
    % minimum width is what actually determines every node's width, with no exceptions.
    box/.style={draw, rounded corners, minimum width=6.2cm, text width=5.6cm,
...
% network depends on both blockentity and sim directly, so both of the next two edges skip
% over at least one intervening box in the stack (network->blockentity skips "block";
% network->sim skips both "block" and "blockentity") and are routed as nested doglegs out to
% the right rather than straight lines, which would otherwise cut through that intervening
% text -- the inner dogleg sits closer to the column, the outer one further out, so the two
% arrows run parallel without crossing. (An earlier fix only rerouted the outer one, having
% missed that the inner network->blockentity edge has exactly the same problem.)
\draw[-{Latex[length=2mm]}] (network.east) -- ++(0.7,0) |- (blockentity.east);
\draw[-{Latex[length=2mm]}] (network.east) -- ++(1.4,0) |- (sim.east);
...
electrical role: six circuit elements (Figure~\ref{fig:elements}: resistor, capacitor,
inductor, memristor, diode, and an ideal op-amp -- the mod's only three-terminal component,
its input-pair keyed as a separate lead from its output without any per-terminal-count
branching in the algorithm above); two purely topological blocks (Figure~\ref{fig:wireground}:
a wire, conductive on all six faces with no orientation, and a ground block, identical to wire
for topology but additionally establishing the network's voltage reference); three
independent voltage sources (Figure~\ref{fig:generators}: a fixed-DC power supply, a function
generator producing sine/square/triangle waves whose amplitude and frequency are overridden
by two attachable modules, and a dedicated AC Source used only by the Bode-plot probe below,
Section~\ref{sec:ac-analysis}) -- all three require a redstone signal before driving anything
(Section~\ref{sec:pedagogy}); and four measurement tools (Figure~\ref{fig:probes}: an
in-circuit ammeter, electrically an ideal 0~V source reporting an exact branch current, plus
three handheld probes described next).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{elements_gallery.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.45\columnwidth]{wire_ground_gallery.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.95\columnwidth]{generators_gallery.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.65\columnwidth]{probes_gallery.png}
