...
% NOTE (author's note, delete before submission): this screenshot predates the diode,
% ideal op-amp, and X-Y probe and needs to be retaken in-game to show all fifteen items.
\begin{figure}[htbp]
\centering
...
oscilloscope provides. Both axes share a single scale, set by the larger of the two
channels' peak magnitude, so that the resulting shape's true aspect ratio is preserved --
for instance, two equal-amplitude sinusoids in quadrature trace a circle rather than a
distorted ellipse. This mode is pedagogically distinct from the time-domain probe: it makes
the relative phase and amplitude between two signals directly visible as a shape, rather
than requiring a player to compare two scrolling traces by eye.
